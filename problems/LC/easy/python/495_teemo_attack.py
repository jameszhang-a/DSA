...
If Teemo attacks again before the poison effect ends, the timer for it is reset, and the
...
class Solution:
    def findPoisonedDuration(self, timeSeries: List[int], duration: int) -> int:
        ...
        intuition: merge intervals
        - linear traversal, add each duration if no conflict,
        if there is conflict, add the difference between next start and this start
        """
        total = 0
    ...
    def test(self, inputs, outputs):
        for i, input in enumerate(inputs):
            if (out := self.findPoisonedDuration(*input)) != outputs[i]:
                print(f"{i} failed")
                print(f"Output:	 {out}")
